...
class Transaction {
  async addPaymentPhone(req, res) {
    try {
      ...
      const paymentDetails = {
        ...
        redirectMode: "POST",
        callbackUrl:
          "https://daily-dish.onrender.com/api/user/payment-callback",
        mobileNumber: Mobile,
        paymentInstrument: {
          type: "PAY_PAGE",
        },
  ...
  async paymentcallback(req, res) {
    ...
    console.log(
      `Callback received: Transaction ${merchantTransactionId}, Status: ${state}`
    );
    ...
    if (data) {
      ...
      if (state === "COMPLETED" && data.config) {
        ...
        await CartModel.findOneAndUpdate(
          { userId: data.userId, status: "Added" },
          { $set: { status: state } }
        );
      }
      await data.save();
